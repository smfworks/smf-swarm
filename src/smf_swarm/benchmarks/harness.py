...
class BenchmarkReport:
    ...
    def to_markdown(self, path: str) -> None:
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write("## Metrics\n\n")
            f.write("| Mode | Brier ↓ | ECE ↓ | MCE ↓ | Accuracy ↑ | Precision ↑ | Recall ↑ | F1 ↑ | Avg Dur(s) |\n")
            f.write("|------|---------|-------|-------|------------|-------------|----------|------|-----------|\n")
...
class BenchmarkHarness:
    ...
    def run(
        ...
        Returns:
            BenchmarkReport with all metrics and plots.
        """
        t0 = time.time()
        ...
        for mode in modes:
            for ms in multi_samples:
                ...
                for i, rec in enumerate(records, 1):
                    if i % 10 == 0 or i == 1:
                        print(f"  [{i}/{len(records)}] {rec['question_text'][:60]}...")
                    ...
                    # Record to backtest store with benchmark_run_id tag
                    try:
                        self.store.record(
                            query=rec["question_text"],
                            domain=domain,
                ...
                # Compute metrics
                brier = self._brier(confidences, outcomes)
                ece = self._ece(confidences, outcomes)
                mce = self._mce(confidences, outcomes)
                acc, prec, rec, f1 = self._classification_scores(confidences, outcomes)
                ...
                self._reliability_plot(confidences, outcomes, 10, plot_path)
                print(f"  {label} — Brier: {brier:.4f} | ECE: {ece:.4f} | Acc: {acc:.4f} | AvgDur: {run_metrics[label]['avg_duration_s']:.1f}s")
