...
class ConfidenceOutput(BaseModel):
    ...
    reasoning: str = Field(
        default="", description="Brief reasoning for the confidence score"
    )


class DataQualityOutput(BaseModel):
    data_quality_score: float = Field(
        ..., ge=0.0, le=1.0, description="Quality of data sources"
    )
    key_sources: list[str] = Field(
        default_factory=list, description="Named data sources"
    )
...
class FeatureOutput(BaseModel):
    feature_count: int = Field(
        ..., ge=1, description="Number of predictive features identified"
    )
    top_features: list[str] = Field(
        default_factory=list, description="Names of top features"
    )


class ReportSectionsOutput(BaseModel):
    executive_summary: str = Field(
        default="", description="Concise 1-paragraph summary"
    )
