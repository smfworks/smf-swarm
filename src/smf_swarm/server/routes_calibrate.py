...
async def calibrate(
    req: CalibrationRequest, auth=Depends(get_auth), rl=Depends(get_ratelimiter)
):
    ...
    try:
        ...
        return CalibrationResponse(
            empirical_coverage=(
                round(empirical_cov, 4) if empirical_cov is not None else 0.0
            ),
            ...
            recommended_shift=(
                round(cp.recommended_shift, 4)
                if hasattr(cp, "recommended_shift")
                else None
            ),
        )
    except ImportError:
        ...
        margin = (
            1.96 * (empirical_coverage * (1 - empirical_coverage) / n) ** 0.5
            if n > 1
            else 0.5
        )
